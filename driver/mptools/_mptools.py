    ...
    def main_loop(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            if time.time() > next_time:
                logger.debug(f"TimerProcWorker.main_loop : calling main_func")
                self.main_func()
                next_time = time.time() + self.INTERVAL_SECS
...
class Proc:
    ...
    def terminate(self):
        ...
        if self.proc.is_alive():
            logger.error(f"Proc.terminate failed to terminate {self.name} after {NUM_TRIES} attempts")
            return False
        else:
            logger.info(f"Proc.terminate terminated {self.name} after {NUM_TRIES - tries} attempt(s)")
            return True
