...
from typing import Protocol, TypeVar, Generic, Tuple, Set, Optional, Dict, Iterable
...
from augmentum.type_descs import (
    ArrayTypeDesc, FunctionTypeDesc, IntTypeDesc,
    PointerTypeDesc, PrimitiveTypeDesc, RealTypeDesc,
    TypeDesc, StructTypeDesc, UnknownTypeDesc
)
...
class GenerateBodyCall(Protocol):
    ...
    def __call__(self, return_type : str, arg_vals : str,
                   path_code : str, probed_type : str,
                   null_check_id : Optional[str], value_op : Optional[str]) -> str: ...

def generate_register_extension_point(
        sys_prog_src : str, mod_name : str, fn_name : str, orig_type : str,
        orig_identifier : str, mod_identifier : str) -> str:
    return f"""
...
def generate_modified_function(
        function : Function, orig_type : str, orig_id : str, mod_id : str,
        path_code : str, probed_type : str, null_check_id : Optional[str],
        generate_body : GenerateBodyCall,
        value_op : Optional[str]) -> str:
    ...
    function_body = generate_body(return_type, arg_vals, path_code,
                        probed_type.get_cpp_type().get_type_string(),
                        null_check_id, value_op)
...
def fill_extension_template(
        mname : str, fname : str, log_file : str, sys_prog_src : str,
        probe_type : str, forward_decl : str, modified_fn : str
    ) -> str:
...
class StructEntry:
    ...
    def __init__(self, td : StructTypeDesc, deep_traversal : bool):
        ...
        self.forward_decls : Iterable[StructTypeDesc] = [] # forward declarations needed for this type
    ...
    def __str__(self):
        ...
        return f"{self.td} - [{dependencies}] - [{dependents}]" \
...
def build_struct_dependency_graph(td : TypeDesc, struct_graph : Dict[StructTypeDesc, StructEntry],
                                deep_traversal : bool) -> Optional[StructTypeDesc]:
    """
    ...
    if isinstance(td, StructTypeDesc):
        ...
        if (not td in struct_graph or
            (td in struct_graph and deep_traversal and not struct_graph[td].deep_traversal)):
            ...
            for elem in td.elem_types:
                dependency = build_struct_dependency_graph(elem, struct_graph, deep_traversal)
    ...
    if isinstance(td, PointerTypeDesc):
        ...
        if deep_traversal:
            return build_struct_dependency_graph(td.pointee, struct_graph, deep_traversal = False)

    if isinstance(td, ArrayTypeDesc):
        return build_struct_dependency_graph(td.contained_type, struct_graph, deep_traversal)
...
def get_struct_definitions_from_graph(struct_graph : Dict[StructTypeDesc, StructEntry]) -> str:
    """
...
def generate_path_code(path : augmentum.paths.Path, function : Function, path_code_id : str) \
    ...
    for idx, elem in enumerate(path_elems):
        if elem == 'Z':
            ...
            cast = ""
            if path_type.get_cpp_type().get_type_string() != path.type.get_cpp_type().get_type_string():
                # should only happen if a float sits in an int32
                assert isinstance(path_type, IntTypeDesc) and path_type.bits == 32 \
                   and isinstance(path.type, RealTypeDesc) and path.type.bits == 32, "Unexpected path types!"
            ...
            if idx == len(path_elems) - 1: raise RuntimeError("Invalid path format: " + str(path))

            path_type = path_type.pointee

            if null_check_id != None:
            ...
            if path_elems[idx + 1].startswith('S'):
                call = f"{call}->"
            elif path_elems[idx + 1] in ['L','R'] or path_elems[idx + 1].startswith('T-'):
                call = f"*{call}"
            else:
            ...
            if idx == 0: raise RuntimeError("Invalid path format: " + str(path))
    ...
    assert isinstance(path_type, PrimitiveTypeDesc), f"Expected primitive type for final probe type but got: {path_type}"
    return path_code, path_type, null_check_id


def generate_extension_code(log_file : Path, sys_prog_src : Path, function : Function, path : Path,
                            generate_body : GenerateBodyCall,
                            value_op : Optional[str] = None, path_code_id : str = "probed") -> str:

    path_code, probed_type, null_check_id = generate_path_code(path, function, path_code_id)
    modified_function = generate_modified_function(function,
                            "original_t", "original_function", "modified_function",
                            path_code, probed_type,
                            null_check_id, generate_body, value_op)

    extension_code = fill_extension_template(
        function.module, function.name, str(log_file), str(sys_prog_src),
        probed_type.get_cpp_type().get_type_string() if isinstance(probed_type, RealTypeDesc) else "int64_t",
...
class NullProbe(PriorProbe):
    ...
    def get_extension_body(self, return_type : str, arg_vals : str, path_code : str,
                           probed_type : str, null_check_id : Optional[str],
                           _ : Optional[str] = None) -> str:
        if return_type == "void":
...
"""
    ...
    def extension_code(self, log_file : Path, sys_prog_src : Optional[Path]) -> str:
        assert sys_prog_src is not None, "Given system program source path must not be None."

        return generate_extension_code(log_file, sys_prog_src, self.function, self.path,
                    self.get_extension_body, path_code_id=NullProbe.ID_TMPL)

    def get_description(self) -> str:
        return f"Null Probe for function {self.function}" + "\n" + \
...
class ParameterProbe(PriorProbe, Generic[T], ABC):
    ...
    def __init__(self, function: Function, path: augmentum.paths.Path, priorId : str, value: T):
        super().__init__(function, path, priorId)
        self.value = value
    ...
    def get_extension_body(self, return_type : str, arg_vals : str, path_code : str,
                           probed_type :str, null_check_id : Optional[str],
                           value_op : Optional[str] = None) -> str:
        if return_type == "void":
            return_stmt, orig_return = "",""
        else:
            return_stmt = "return r;"
...
"""
    ...
    def get_description(self) -> str:
        return f"{self.get_value_description()} Probe for function {self.function}" + "\n" + \
               f"in module {self.function.module} " + "\n" \
...
class StaticProbe(ParameterProbe):
    ...
    def extension_code(self, log_file : Path, sys_prog_src : Optional[Path]) -> str:
        ...
        return generate_extension_code(log_file, sys_prog_src, self.function,
                                       self.path, self.get_extension_body)
...
class OffsetProbe(StaticProbe, Generic[T]):
    ...
    def extension_code(self, log_file : Path, sys_prog_src : Optional[Path]) -> str:
        ...
        return generate_extension_code(log_file, sys_prog_src, self.function, self.path,
                                       self.get_extension_body, value_op="+")
