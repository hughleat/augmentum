...
from typing import Optional, Iterable, Any, Dict, Set, Tuple
...
from augmentum.probes import OffsetProbe, PriorProbe, ScaleProbe, StaticProbe, NullProbe
...
def get_real_limit(bits : int) -> Tuple[float,float]:
    ...
    if bits == 32:
        return 1.17549e-38, 3.40282e+38
    if bits == 64:
        return 2.22507e-308, 1.79769e+308
...
class ProbeResult:
    ...
    def is_exec_success(self) -> bool:
        return self.compile_ok == ExecutionResult.SUCCESS and \
            self.run_ok == ExecutionResult.SUCCESS and \
            self.verify_ok == ExecutionResult.SUCCESS
    ...
    def __str__(self) -> str:
        ...
        if self.objective is not None:
            obj_supp = ' '.join('='.join((k,str(v))) for (k,v) in self.objective.supplementary.items())

        return str(self.probe) + "\n" \
            + "test_case: " + str(self.test_case) + "\n" \
            + "compile: " + str(self.compile_ok) + "\n" \
            + "run: " + str(self.run_ok) + "\n" \
            + "verify: " + str(self.verify_ok) + "\n" \
            + "compile_t: " + time_format(self.compile_time) + "\n" \
            + "run_t: " + time_format(self.run_time) + "\n" \
            + "objective: " + str(self.objective) + "\n" \
            + "relative objective: " + str(self.rel_objective) + "\n" \
            + "objective supp: " + str(obj_supp) + "\n" \
            + "extension path: " + str(self.ext_path) + "\n" \
            + "# exec_log: " + str(len(self.exec_log)) + "\n"
...
class RandomSampleSearch(SampleSearch):
    ...
    def __init__(self, targets: Iterable, max_samples : int):
        ...
        if max_samples < 0:
            logger.error(f"Maximum samples to be chosen for search strategy must be larger or equal zero but is {max_samples}.")
            max_samples = 0
...
class VerifySampleSearch(SampleSearch):
    ...
    def update_search(self, prev_success : bool):
        ...
        """
        assert self.next_target > 0 and self.next_target <= len(self.targets), \
...
class BinaryBoundsSearch(SearchStrategy, ABC):
    ...
    def __init__(self, min_bound : Number, max_bound : Number,
                 lower : Optional[Number] = None, upper : Optional[Number] = None,
        ...
        if lower is not None and upper is not None:
            ...
            self.lower, self.upper, self.min_bound, self.max_bound = \
                 BinaryBoundsSearch.find_initial_bounds(initial_values, self.min_bound, self.max_bound)
    ...
    @staticmethod
    def find_initial_bounds(tests : Dict[Number,bool],
        initial_min_bound : Number, initial_max_bound : Number) \
        ...
        while ui < n and li < n - 1:
            ...
            if (not ((tests[keys[li]] is True and li == 0) or
                     (tests[keys[li]] is False and tests[keys[li+1]] is True))):
                li += 1
                ui = li
            # look for change from True to False for upper (or if at True last index)
            elif (not ((tests[keys[ui]] is True and ui == n - 1) or
                       (tests[keys[ui]] is True and tests[keys[ui+1]] is False))):
                ui += 1
            else:
                ...
                if max_diff is None or max_diff < curr_diff:
                    ...
                    max_lower_idx = li if li == 0 and tests[keys[li]] is True else li+1
                    max_upper_idx = ui
    ...
    def initialise_verification(self):
        ...
        logger.debug(f"Binary Search Verification started between {self.lower} and {self.upper} with probe targets {targets}")
    ...
    def handle_verify_bounds(self):
        ...
        """
        assert self.verify_search is not None, "Search verification expected to be initialised."
    ...
    def next_search_target(self) -> Optional[Number]:
        assert self.target is not None, "Target not expected to be null if search is not done."
...
class BinarySearch(SearchStrategy, ABC):
    ...
    def initialise(self, max_bound : Number, min_bound : Number, start_bound : Number):
        ...
        logger.debug(f"Binary Search initialise for bounds min {min_bound} start {start_bound} max {max_bound}.")
        ...
        if (max_bound < start_bound or
            min_bound > start_bound or
            min_bound > max_bound or
            min_bound == start_bound):
            self.invalid = True
    ...
    def _select_next_value(self):
        ...
        if self.target.eval_success:
            if self.center is None:
                ...
                self.left = self.center
                self.center = self.split_interval(self.left, self.right) # L + ((R - L) // 2)
                self.target = SearchTarget(self.center)
        else:
            if self.center:
    ...
    def next_search_target(self) -> Optional[Number]:
        assert self.target is not None, "Target not expected to be null when requesting next search target."
        return self.target.value
...
class RealBinaryBoundsSearch(BinaryBoundsSearch):
    ...
    def get_verification_targets(self, lower : Number, upper : Number) -> Optional[Iterable[Number]]:
        if not math.isfinite(lower) or not math.isfinite(upper):
            logger.warning(f"Non finite number for lower {lower} or upper {upper} bound received.")
            return None
        ...
        if (upper - lower == float('inf')): # avoid going out of float bounds
            targets = unique_random_reals(upper - sys.float_info.max, upper, BIN_VERIFY_SAMPLES)
        else:
...
class SamplePrior(Prior, ABC):
    ...
    def select_next_probe(self) -> PriorProbe:
        ...
        assert self.curr_value is not None, "Expected available value if search is not done."
    ...
    def update(self, probe_result: ProbeResult):
        ...
        assert self.curr_value is not None, "Expected available value if update was called."
...
class RangePrior(Prior, ABC):
    ...
    def __init__(self, function: Function, path: Path, initial_values : Dict[Number, bool]):
        super().__init__(function, path)
    ...
    def prior_result(self) -> Optional[PriorResult]:
        ...
        if self.is_done():
            success = \
                not self.is_invalid() and \
                not self.search_strat.has_next_target() and \
                self.search_strat.lower != self.search_strat.upper # single value does not count as range
...
class BoundedPrior(Prior, ABC):
    ...
    def __init__(self, function: Function, path: Path,
                 lower_search_strat : BinarySearch, upper_search_strat : BinarySearch):
        super().__init__(function, path)
    ...
    def is_done(self) -> bool:
        ...
        while self.next_probe is None and self.state != BoundedPrior.SearchState.DONE:
            ...
            if self.state == BoundedPrior.SearchState.SEARCH_UPPER:
                if self.upper_verify_success is False and self.upper_search_strat.has_next_target():
            ...
            if self.state == BoundedPrior.SearchState.SEARCH_LOWER:
                if self.lower_verify_success is False and self.lower_search_strat.has_next_target():
            ...
            if self.state == BoundedPrior.SearchState.INIT_VERIFICATION:
                def init_verification(prev_success : bool, search : BinarySearch) -> VerifySampleSearch:
                ...
                self.upper_verify_strat = init_verification(self.upper_verify_success, self.upper_search_strat)
                self.lower_verify_strat = init_verification(self.lower_verify_success, self.lower_search_strat)

                self._set_state(BoundedPrior.SearchState.UPPER_VERIFICATION)

            if self.state == BoundedPrior.SearchState.UPPER_VERIFICATION:
                if self.upper_verify_strat is not None and self.upper_verify_strat.has_next_target():
            ...
            if self.state == BoundedPrior.SearchState.LOWER_VERIFICATION:
                if self.lower_verify_strat is not None and self.lower_verify_strat.has_next_target():
            ...
            if self.state == BoundedPrior.SearchState.EVAL_VERIFICTAION:
                ...
                def eval_verification(verify_strat : Optional[VerifySampleSearch], search_start : BinarySearch) -> bool:
                    ...
                    for idx, value in enumerate(targets):
                        if vres[value] is not True:
                            first_fail_idx = idx
                            break
                    assert first_fail_idx is not None, "Failing value expected in verification results"
                ...
                self.upper_verify_success = \
                    eval_verification(self.upper_verify_strat, self.upper_search_strat)
                self.lower_verify_success = \
                    eval_verification(self.lower_verify_strat, self.lower_search_strat)
    ...
    def prior_result(self) -> Optional[PriorResult]:
        ...
        if self.is_done():
            ...
            success = \
                not self.is_invalid() and \
                self.state == BoundedPrior.SearchState.DONE and \
                (lower != 0 or upper != 0) # zero values are no actual offsets
...
class CompositePrior(Prior):
    ...
    def is_done(self) -> bool:
        ...
        if (self.state == CompositePrior.PriorState.NULL and
            self.current_prior.is_invalid()):
            return True

        # select the next one if we are done with the current but
        # still have work to do overal
        while (self.state != CompositePrior.PriorState.DONE and
               self.current_prior.is_done()):
    ...
    def _select_next_prior(self):
        ...
        if self.state == CompositePrior.PriorState.NULL:
            assert isinstance(self.current_prior, NullPrior), \
                "Expected Null prior to be active when in NULL state."

            if (self.skip_immutables and
                not isinstance(self.path, ResultPath) and
                self.current_prior.mutated_during_call is False):
                # if we skip immutable parameter paths and the Null Prior could
                # not detect any mutation, skip to the end
                logger.info(f"Immutable function parameter path suspected. Skipping Path {self.path}")
                ...
                # or if we need to be original aware (OA)
                    ...
                    self._set_current_prior(
                        BroadCastPrior(self.function, self.path,
                            MinMaxSampleSearch(targets, CompositePrior.MAX_BROADCAST_SAMPLES))
                    )
            ...
            assert isinstance(self.current_prior, BroadCastPrior), \
                "Expected Boradcast prior to be active when in BROADCAST state."

            if (self.current_prior.prior_result() is not None and
                self.current_prior.prior_result().success):
                ...
                probed_values = set(self.priors[0].probed_values.keys()) # first prior is NullPrior

                if isinstance(self.path.type, IntTypeDesc):
                    assert self.path.type.bits > 1, "Boolean path type not expected at this point."
                    self._set_current_prior(
                    ...
                    raise ValueError(f"Unexpected terminal path type during prior evaluation {self.path.type}.")
            ...
            if (self.current_prior.prior_result() is None or
                not self.current_prior.prior_result().success):
                self._set_current_prior(
                    IntegerRangePrior(self.function, self.path, self.current_prior.probed_values))
            ...
            assert isinstance(self.current_prior, IntegerRangePrior), \
            ...
            assert isinstance(self.current_prior, AllRealsPrior), \
            ...
            if (self.current_prior.prior_result() is None or
                not self.current_prior.prior_result().success):
                self._set_current_prior(
                    RealRangePrior(self.function, self.path, self.current_prior.probed_values))
            ...
            assert isinstance(self.current_prior, RealRangePrior), \
            ...
            assert isinstance(self.current_prior, (IntegerOffsetPrior, RealOffsetPrior)), \
            ...
            if isinstance(self.current_prior, IntegerOffsetPrior):
                ...
                raise ValueError("Expected Offset Prior to be active when in OA_OFFSET state.")
            ...
            assert isinstance(self.current_prior, (IntegerScalePrior, RealScalePrior)), \
    ...
    def prior_result(self) -> Optional[PriorResult]:
        ...
        any_success = any(
            (p.prior_result() is not None and p.prior_result().success for p in self.priors[1:])
        )
...
class NullPrior(Prior):
    ...
    def _save_probed_values(self, probe_result: ProbeResult, probed_values : Dict[Number, int]):
        """
        ...
        for entry in probe_result.exec_log:
            ...
            if not probed_num in probed_values:
    ...
    def update(self, probe_result: ProbeResult):
        ...
        if probe_result.is_exec_success() and len(probe_result.exec_log) > 0:
            ...
            if len(self.probed_values) == 0:
                logger.warning(f"No valid Null Probe results could be parsed for {self.function} {self.path} {probe_result.test_case}")
                self.invalid = True
        else:
            ...
            # logged.
            logger.warning(f"Null Probe execution failed for {self.function} {self.path} {probe_result.test_case}")
            self.invalid = True
    ...
    def prior_result(self) -> Optional[PriorResult]:
        ...
        """
        if self.executed:
            values = NullPrior.DATA_DELIM.join([f"{n},{f}" for n,f in self.probed_values.items()])
            data_str = f"MUTATED:{self.mutated_during_call}{NullPrior.DATA_DELIM}DATA:{values}"
...
class BroadCastPrior(SamplePrior):
    ...
    def __init__(self, function: Function, path: Path, search_strat : SampleSearch):
        ...
        logger.info("BroadCastPrior prior created with probe targets " + str(self.search_strat.targets))
...
class AllIntegersPrior(SamplePrior):
    ...
    def __init__(self, function: Function, path: Path, initial_probes : Set[Number]):
        ...
        super().__init__(function, path, SampleSearch(targets), probed_values, min_values_checked)
...
class AllRealsPrior(SamplePrior):
    ...
    def __init__(self, function: Function, path: Path, initial_probes : Set[Number]):
        ...
        if (max_pos - max_neg == float('inf')):  # avoid going out of float bounds
            prelim_targets = unique_random_reals(max_pos - sys.float_info.max, max_pos, ALL_PRIOR_SAMPLES)
        else:
...
class IntegerRangePrior(RangePrior):
    ...
    def __init__(self, function: Function, path: Path, initial_probes : Dict[Number,bool]):
        super().__init__(function, path, initial_probes)

        logger.debug(f"Integer Range Prior created with bounds [{self.search_strat.lower},{self.search_strat.upper}]")
...
class RealRangePrior(RangePrior):
    ...
    def __init__(self, function: Function, path: Path, initial_probes : Dict[Number,bool]):
        ...
        logger.debug(f"Real Range Prior created with bounds [{self.search_strat.lower},{self.search_strat.upper}]")
...
class IntegerOffsetPrior(OffsetPrior):
    ...
    def _get_verification_targets(self, search : BinarySearch) -> Optional[Iterable[Number]]:
        ...
        targets = unique_random_integers(0, search.final_bound, samples, open_interval=True)

        logger.debug(f"Binary Search Verification started between 0 and {search.final_bound} with probe targets {targets}")
        return targets
    ...
    def prior_result(self) -> Optional[PriorResult]:
        ...
        if self.is_done():
            ...
            success = \
                not self.is_invalid() and \
                self.state == BoundedPrior.SearchState.DONE and \
                (not math.isclose(lower, 0) or not math.isclose(upper, 0)) # zero offset is simply to original value
