...
from sqlalchemy import Table, Column, String, MetaData, Integer, ForeignKey, Enum, Boolean
...
class HeuristicDB:
    ...
    def __init__(self, url : str):
        ...
        self.pheu_tbl = Table(HeuristicDB.PATH_HEURISTIC_TBL, self.metadata,
            ...
            Column('prior_success', String, nullable=False), # working prior found (other than NullPrior)
            Column('obj_improvement', Boolean, nullable=False), # obj improvement found
            Column('test_coverage', Integer, nullable=True) # number of tests covering this entry
        )

        # probe log table
        self.probe_tbl = Table(HeuristicDB.PROBE_TBL, self.metadata,
            Column('id', Integer, primary_key=True, autoincrement=True, nullable=False),
            Column('module', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.module"), nullable=False),
            Column('function', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.function"), nullable=False),
            Column('path', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.path"), nullable=False),
        ...
        # prior result table
        self.prior_tbl = Table(HeuristicDB.PRIOR_TBL, self.metadata,
            Column('module', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.module"), primary_key=True, nullable=False),
            Column('function', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.function"), primary_key=True, nullable=False),
            Column('path', String, ForeignKey(f"{HeuristicDB.PATH_HEURISTIC_TBL}.path"), primary_key=True, nullable=False),
    ...
    def add_path_heuristic(self, m_name : str, fn_name : str, path : Path,
                           presult : Optional[PriorResult], obj_improvement : bool, test_coverage : int):
        """Add function path entry or update if already exists"""
        prior_success = None
        if presult is not None:
            prior_success = presult.success
        self.add_path_heuristic_entry(m_name, fn_name, str(path), prior_success, obj_improvement, test_coverage)


    def add_path_heuristic_entry(self, m_name : str, fn_name : str, path : str,
                                 prior_success : Optional[bool],
                                 obj_improvement : bool, test_coverage : int) -> bool:
    ...
    def add_probelog(self, module : str, function : str, path : Path,
                     probe_log : Iterable[ProbeResult], record_exec_log : bool = False):
        """Add probe results from given prior to database."""
        for probe_result in probe_log:

            probe_log = "#".join( [",".join(log) for log in probe_result.exec_log]) if record_exec_log else None

            obj_supp = None
            if probe_result.objective is not None:
                obj_supp = '#'.join('='.join((k,str(v))) for (k,v) in probe_result.objective.supplementary.items())
            ...
            self.add_probelog_entry(
                ...
                str(probe_result.probe.get_probe_value()) if probe_result.probe.get_probe_value() is not None else None,
                probe_result.compile_ok,
                probe_result.run_ok,
                probe_result.verify_ok,
                probe_result.compile_time,
                probe_result.run_time,
                str(probe_result.objective.score) if probe_result.objective is not None else None,
                str(probe_result.rel_objective) if probe_result.rel_objective is not None else None,
                obj_supp,
                probe_result.ext_path,
                probe_log
            )

    def add_probelog_entry(self, module : str, function : str, path : str,
        case : str, prior : str, value : Optional[str], compile_ok : ExecutionResult,
        run_ok : ExecutionResult, verify_ok : ExecutionResult,
        compile_t : Optional[float], run_t : Optional[float], objective : Optional[str],
        rel_objective : Optional[str], objective_supp : Optional[str],
        ext_path : Optional[pathlib.Path], exec_log : Optional[str]) -> bool:
    ...
    def add_prior_results(self, m_name : str, fn_name : str, path : Path,
                          priors : Iterable[Prior]):
        """Add prior results from given sequence of priors to database."""
        for p in priors:
            presult = p.prior_result()
            if presult != None:
                self.add_prior_results_entry(m_name, fn_name, str(path),
                        str(p), presult.success, str(presult.result_data))

    def add_prior_results_entry(self, m_name : str, fn_name : str, path : str,
                                prior : str, prior_success : bool, prior_data : str):
