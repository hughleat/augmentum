...
from augmentum.function import (
    parse_collected_function_stats,
    use_relative_src_path
)
...
class SysProgBuilder(ABC):
    ...
    instr_args_target_template = \
        "   -mllvm -target-functions={target_functions}"
    ...
    def __init__(self, tools : Dict[str,Any],
        ...
        self.instr_interface = InstrumenterInterface(self.target_functions_p, self.src_path)
    ...
    def collect_functions(self) -> Optional[Dict[str,str]]:
        ...
        if self.run_build_cmd(instr_args, module = self.instrument_module, clean_up=True):
            data_paths = parse_collected_function_stats(self.build_path / "statistics", self.build_path)
...
class LLVMBuilder(SysProgBuilder):
    ...
    def __init__(self, tools : Dict[str,Any], sys_prog_conf : Dict[str,Any],
                 sys_prog_src_dir : Path, sys_prog_bld_dir : Path,
                 cpus : Optional[int] = None, verbose : bool = False):
        ...
        if "gcc_install_prefix" in sys_prog_conf:
            self.gcc_install_prefix = f"-DGCC_INSTALL_PREFIX={sys_prog_conf['gcc_install_prefix']}"
    ...
    def run_build_cmd(self, instr_args : str, module : str = "", clean_up = False) -> bool:
        ...
        bld_cmd = LLVMBuilder.build_template.format(
            instrumented_dir = str(self.instrumented_p),
            module = module,
            cpus = str(self.cpus) if self.cpus != None else "`nproc`"
        )
...
def create_builder(tools : Dict[str,Any], sys_prog_conf : Dict[str,Any],
                   sys_prog_src_dir : Path, sys_prog_bld_dir : Path,
                   cpus : Optional[int] = None, verbose : bool = False) -> SysProgBuilder:
    if sys_prog_conf["type"] == "llvm":
        return LLVMBuilder(tools, sys_prog_conf, sys_prog_src_dir, sys_prog_bld_dir, cpus, verbose)
    if sys_prog_conf["type"] == "synth-heuristic":
        return HeuristicSynthBuilder(tools, sys_prog_src_dir, sys_prog_bld_dir, cpus, verbose)
    else:
        raise RuntimeError("Unsupported system program builder type: " + str(sys_prog_conf["type"]))
