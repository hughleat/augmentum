...
from typing import Iterable, Dict, Callable, Any, Generator
...
class NamedStructData:
    ...
    def __init__(self, module_name: str, struct_name: str, serialised_type: str, llvm_name: str, extra: str):
    ...
    def extract_packed(self, data : str):
        ...
        elems = data.split("#")
        assert len(elems) == 4, "Unexpected format for extra struct information: " + data
    ...
    def __str__(self):
        return self.key + " " + self.module_name + " " + self.struct_name + " " + \
               self.serialised_type + " " + self.llvm_name
...
class FunctionData:
    ...
    def __init__(self, module_name: str, function_name: str, serialised_type: str, demangled_name: str,
                 instruction_count : int, can_instrument : str):
        self.module_name = module_name
    ...
    def __str__(self):
        return self.module_name + " " + self.function_name + " " + \
               self.serialised_type + " " + self.function_name_demangled

def load_target_function_stats(function_file : Path) -> Generator[FunctionData, None, None]:
    """
...
def parse_collected_function_stats(src_dir : Path, target_dir : Path) -> Dict[str,str]:
    ...
    if not src_dir.exists():
        raise RuntimeError("Source directory invalid when merging function stats: " + str(src_dir))
    if not target_dir.exists():
        raise RuntimeError("Target directory invalid when merging function stats: " + str(target_dir))
...
class Function:
    ...
    """
    def __init__(self, module: str, name: str, type: FunctionTypeDesc, function_stats: FunctionData):
        self.module = module
        self.name = name
        self.type = type
    ...
    def __str__(self) -> str:
        return str(self.type.return_type) + " " + self.name + "(" + ",".join(map(str, self.type.arg_types)) + ")"
