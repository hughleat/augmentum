...
from augmentum.type_descs import FunctionTypeDesc, PointerTypeDesc, PrimitiveTypeDesc, StructTypeDesc, TypeDesc, UnknownTypeDesc
...
def is_stl_function(mangled_name : str) -> bool:
    return re.match(STL_REGEX, mangled_name) != None

def is_dtor(mangled_name : str) -> bool:
    return re.match(DTOR_REGEX, mangled_name) != None
...
def has_unknown_types(td: TypeDesc, structs_with_unknown_elements : Dict[str,bool], is_dereffed : bool = False) -> bool:
    ...
    if isinstance(td, FunctionTypeDesc):
        is_unknown = has_unknown_types(td.return_type, structs_with_unknown_elements, is_dereffed)
        for a in td.arg_types:
            if is_unknown: break
            is_unknown = is_unknown or has_unknown_types(a, structs_with_unknown_elements, is_dereffed)
        return is_unknown
        ...
        for e in td.elem_types:
            if is_unknown: break
            is_unknown = is_unknown or has_unknown_types(e, structs_with_unknown_elements, is_dereffed)
        ...
        if is_dereffed: # do not recurse deeper if already dereffed
            ...
            return has_unknown_types(td.pointee, structs_with_unknown_elements, is_dereffed=True)
...
class InstrumentPrimitiveReturn(InstrumentDefault):
    ...
    """
    def should_instrument(self, function : Function) -> bool:
        return super().should_instrument(function) and \
            isinstance(function.type.return_type, PrimitiveTypeDesc)
...
class InstrumentFunctionList(FunctionFilter):
    ...
    def __init__(self, allow_list : Iterable[Tuple[str,str]]):
        ...
        for entry in allow_list:
            module = entry[0]
            function = entry[1]
            if not module in self.allow_list:
                self.allow_list[module] = set()
            self.allow_list[module].add(function)

    def should_instrument(self, function : Function) -> bool:
        if function.module in self.allow_list and \
           function.name in self.allow_list[function.module]:
            return True
        else:
            return False