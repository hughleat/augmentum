...
class TypeDesc:
    def get_paths(self, ctx: 'augmentum.paths.PathContext') -> Iterable['augmentum.paths.Path']:
...
class IntTypeDesc(PrimitiveTypeDesc):
    ...
    def get_paths(self, ctx: 'augmentum.paths.PathContext') -> Iterable['augmentum.paths.Path']:
        ...
        if self.bits == 64 or self.bits == 32 or self.bits == 16:
            ...
            ps.extend([augmentum.paths.SplitIntLeftPath(p) for p in half_type.get_paths(ctx)])
            ps.extend([augmentum.paths.SplitIntRightPath(p) for p in half_type.get_paths(ctx)])
    ...
    def get_cpp_type(self, is_dereffed : bool = False) -> CppType:
        if self.bits == 1: cpp_type = CppType("bool")
        elif self.bits in [8,16,32,64]: cpp_type = CppType(f"int{self.bits}_t")
        else:
            raise NotImplementedError(f"Integer type with {self.bits} not implemented yet.")
...
class RealTypeDesc(PrimitiveTypeDesc):
    ...
    def get_cpp_type(self, is_dereffed : bool = False) -> CppType:
        if self.bits == 32: cpp_type = CppType("float")
        elif self.bits == 64: cpp_type = CppType("double")
        else:
            raise NotImplementedError(f"Real type with {self.bits} not implemented yet.")
...
class PointerTypeDesc(TypeDesc):
    ...
    def get_paths(self, ctx: 'augmentum.paths.PathContext') -> Iterable['augmentum.paths.Path']:
        if ctx == augmentum.paths.PathContext.ARG:
            return [augmentum.paths.DerefPath(p) for p in self.pointee.get_paths(augmentum.paths.PathContext.DEREFFED)]
        return []
...
class StructTypeDesc(TypeDesc):
    ...
    def __init__(self, module : str, name: str, forward: bool = False, packed: bool = False, *elem_types: TypeDesc):
        self.__module = module
        self.__name = name
        self.__augmentum_name = f"__augmentum__struct_{StructTypeDesc.next_augmentum_struct_id}"
    ...
    def set_element_types(self, *elem_types: TypeDesc):
        ...
        if self.is_forward():
            ...
            assert self.elem_types == elem_types, "Cannot set element types to a different value"
...
class FunctionTypeDesc(TypeDesc):
    ...
    def get_paths(self, ctx: 'augmentum.paths.PathContext' = None) -> Iterable['augmentum.paths.Path']:
        # only get paths for function context
        if ctx == augmentum.paths.PathContext.FUNCTION:
            ps = []
            ps.extend([augmentum.paths.ResultPath(p) for p in self.return_type.get_paths(augmentum.paths.PathContext.RESULT)])
            for i, t in enumerate(self.arg_types):
                ps.extend([augmentum.paths.ArgumentPath(i, p) for p in t.get_paths(augmentum.paths.PathContext.ARG)])
            return ps
        else:
            return []
