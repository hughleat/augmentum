...
class ObjectiveResult:
    ...
    """
    def __init__(self, score : Any, unit : str = None, supplementary : Optional[Dict] = None):
        self.__score = score
        self.__unit = unit
...
class ObjectiveMetric(ABC):
    ...
    @abstractmethod
    def measure(self, test_case : TestCase, memory_limit : Optional[int] = None) -> Tuple[ExecutionResult, ObjectiveResult]:
        """
...
class CodeSizeObjective(ObjectiveMetric):
    ...
    def measure(self, test_case : TestCase, memory_limit : Optional[int] = None) -> Tuple[ExecutionResult, Optional[ObjectiveResult]]:
    ...
    def get_code_size(self, binary_path : Path) -> ObjectiveResult:
        ...
        for lnum, line in enumerate(stdout.splitlines()):
            if lnum == 1:
                ...
                if len(elems) < 6:
                    logger.error("Object file segments could not be parsed when measuring binary size!")
                    logger.info(stdout)
                    return None
        ...
        supp = {"text":text, "data":data, "bss":bss, "file":file_size, "bz2":bz2_file_size}
        return ObjectiveResult(text_data_size, CodeSizeObjective.result_unit, supplementary=supp)
