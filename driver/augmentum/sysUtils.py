...
async def run_command_async(command : str, timeout : Optional[float] = None,
                            verbose : bool = False,
                            memory_limit : Optional[int] = None,
                            debug_log : Callable[[str], None] = logger.debug) -> Tuple[int,str]:
    ...
    process = await asyncio.create_subprocess_shell(command,
            ...
            preexec_fn=os.setsid, limit=(1024 * 512))
    ...
    while True:
        ...
        try:
            ...
            logger.warning("ValueError while reading stdout lines "
...
def run_command(command : str, timeout : Optional[float] = None,
                verbose : bool = False, memory_limit : Optional[int] = None,
                debug_log : Callable[[str], None] = logger.debug) -> Tuple[int,str]:
    """
...
def try_create_dir(path : Path, use_time : bool = False, tries : int = 100) -> Optional[Path]:
...
def check_arg_list(parser, value : Any, expected : Iterable[Any],
                   error : str = "Invalid cmd argument."):
    """
    Intended to be used for argument parsing with argparse library.
...
def unique_random_integers(start : int, end : int, num : int,
                           max_tries : int = 100, open_interval = False) -> Iterable[int]:
    """
...
def unique_random_reals(start : float, end : float, num : int,
                        max_tries : int = 100) -> Iterable[float]:
    """
