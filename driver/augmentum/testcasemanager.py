...
class TestCaseManager:
    ...
    def __init__(self, tools : Dict[str, Any], wd_workers : Path, keep_probes : bool = False):
        self.tools = tools
        self.wd_workers = wd_workers
        self.keep_probes = keep_probes

    def measure_baseline(self, test_cases : Dict[str, TestCase], objective_metric : ObjectiveMetric,
            baseline_cache : Optional[Path] = None) -> Optional[Dict[str, ProbeResult]]:
        """
        ...
        logger.info(f"Measuring baseline for {len(test_cases)} available test cases ...")
        ...
        bound_probe = BoundProbe(
            self.tools, probe_wd, vanilla_compiler_bins, None,
            objective_metric,
            BaselineProbe(), self.keep_probes, build_extension = False)
        ...
        for tc_name, test_case in test_cases.items():
            ...
            if not probe_result.is_exec_success():
                logger.error(f"Test failed during baseline execution for test {tc_name}" +
                                    "\n" + str(probe_result))
                return None
            ...
            if probe_result.objective.score == 0:
                logger.error(f"Objective baseline is zero for test {tc_name}. "
    ...
    def select_tests(self, target_modules : Set[str], sys_prog : SysProg,
                     test_cases : Dict[str, TestCase]) -> Dict[str, Dict[str, Set[str]]]:
        """
        ...
        """
        logger.info("Selecting test cases for target functions in instrumented module ...")

        probe_wd = try_create_dir(self.wd_workers / "test_select", use_time=True)
        if not probe_wd:
            raise RuntimeError(f"Creating probe working directory failed for {probe_wd}.")

        with sys_prog.bind(TracerProbe("test selection"), probe_wd, self.keep_probes) as bound_probe:
            selected_tests = dict()
            for tc_name, test_case in test_cases.items():
                # compile, execute, verify and measure for each test case
                logger.info("Processing test case " + tc_name + " ...")
                ...
                if not probe_result.is_exec_success():
                    raise RuntimeError(f"Test failed during selection for test {tc_name}" +
                                        "\n" + str(probe_result))
                ...
                for log_entry in probe_result.exec_log:
                    ...
                    if exec_mod in target_modules:
                        ...
                        if not exec_mod in selected_tests:
                            selected_tests[exec_mod] = dict()

                        if not exec_fn in selected_tests[exec_mod]:
                            selected_tests[exec_mod][exec_fn] = set()

                        if not tc_name in selected_tests[exec_mod][exec_fn]:
                            selected_tests[exec_mod][exec_fn].add(tc_name)
                            count += 1
                    else:
                        logger.warning("Instrumentation fragments from different module found: " + exec_mod + " Consider clean build!")
