...
from augmentum.sysUtils import try_create_dir, build_path_or_fail
...
class Profiler:
    def __init__(self, tools : Dict[str, Any], llvm_src_dir : Path,
                 target_compiler_bld : Path, benchmark_factory: BenchmarkFactory,
                 working_dir : Path, verbose : bool):
    ...
    def profile_test_cases(self):
        ...
        profiler_probe = BoundProbe(self.tools, probe_wd, self.target_compiler_bins, None,
                                    None, TracerProbe("benchmark_profiler"), True)
            ...
            with Timer():
                ...
                if probe_result.is_exec_success():
                    ...
                    for entry in probe_result.exec_log:
                        ...
                        if not key in target_functions:
                            target_functions[key] = FnTracePoint(module,function)
                        target_functions[key].count += 1

                else:
                    logger.warning(f"Test case {tc_name} processing failed!\n{probe_result}")
                    probe_result = None
...
def load_config(raw_config : Iterable[str]) -> \
    ...
    b_cfgs = [ config["benchmark"]["available"][b] for b in config["benchmark"]["active"] ]
...
def parse_args():
    ...
    parser.add_argument("--run_id", metavar="ID", type=str, required=True,
                        help="ID used to identify this profiler execution.")
    parser.add_argument("--working_dir", metavar="DIR", type=Path, required=True,
                        help="Directory for generated data.")
    parser.add_argument("--config",  metavar="JSON", type=argparse.FileType('r'), required=True,
    ...
    parser.add_argument("-v","--verbose", action='store_true',
                        help="Activate verbose program output.")
...
def main():
    ...
    with MainContext() as log_ctx:
        ...
        benchmark_factory = BenchmarkFactory(tools_cfg, benchmark_cfgs, verbose = args.verbose)

        bench_profiler = Profiler(tools_cfg, llvm_src_dir, target_compiler_bld,
                                benchmark_factory, wd_run, args.verbose)
