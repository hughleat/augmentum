...
from augmentum.priors import (
    REAL_ACCURACY, REAL_TOLERANCE,
    BinaryBoundsSearch, BinarySearch, BoundedPrior, IntegerBinaryBoundsSearch, IntegerBinarySearch, IntegerOffsetPrior, IntegerRangePrior, IntegerScalePrior,
    ProbeResult, RealBinaryBoundsSearch, RealOffsetPrior, RealRangePrior, RealScalePrior, SearchTarget, get_real_limit, Prior, RangePrior
)
from augmentum.function import Function, FunctionData
from augmentum.probes import OffsetProbe, ScaleProbe, StaticProbe
from augmentum.type_descs import FunctionTypeDesc, i32_t, float_t, i64_t, double_t
...
def prior_update_check(test : unittest.TestCase, prior : Prior):
    ...
    test.assertTrue(search_strat.target.eval_success,
    ...
    test.assertFalse(search_strat.target.eval_success,
        "Updated not registered successful probe as expected.")
    ...
    test.assertFalse(search_strat.target.eval_success,
        "Updating for the same search value twice should require all results to be successful.")

class TestIntervalSearch(unittest.TestCase):

    def check(self, values : Dict[Number, bool], min_bound : Number,
              max_bound : Number, exp_lower : Number,
              exp_upper : Number, exp_min : Number, exp_max : Number):

        lower, upper, min_bound, max_bound = BinaryBoundsSearch.find_initial_bounds(values, min_bound, max_bound)

        self.assertEqual(lower, exp_lower, f"Unexpected lower bound for initial values {values}")
        self.assertEqual(upper, exp_upper, f"Unexpected upper bound for initial values {values}")
        self.assertEqual(min_bound, exp_min, f"Unexpected min_bound bound for initial values {values}")
        self.assertEqual(max_bound, exp_max, f"Unexpected max_bound bound for initial values {values}")
    ...
    def test_bounds(self):
        ...
        self.check({ -5 : True, 4 : True, 5 : False, 8 : True, 34 : True, 40 : True}, -500, 500, 8, 40, 5, 500)

        # # first diff (-5,4) = 9, second diff (4,8) = 4
        self.check({ -5 : True, 4 : False, 5 : True, 8 : False, 34 : False, 40 : False}, -500, 500, -5, -5, -500, 4)

        # # first diff (4,8) = 4, second diff (8,40) = 32
        self.check({ -5 : False, 4 : False, 5 : True, 8 : False, 34 : True, 40 : False}, -500, 500, 34, 34, 8, 40)

        # # first diff (-200,105) = 305, second diff (150,400) = 250
        self.check({-200 : True, 23 : True, 55 : True, 105 : False, 110 : False, 150 : False, 200 : True, 400 : True},
             -2147483648, 2147483647, -200, 55, -2147483648, 105)

        # # first diff (-200,105) = 305, second diff (150,499) = 349
        self.check({-200 : True, 23 : True, 55 : True, 105 : False, 110 : False, 150 : False, 200 : True, 499 : True},
             -2147483648, 2147483647, 200, 499, 150, 2147483647)

class TestIntegerRangeSearch(unittest.TestCase):

    def setUp(self):
        fn_data = FunctionData("add.cpp","_Z3addii",
                               "@$ i32, i32, i32 $@","add(int, int)",2,"instrument")
    ...
    def init_prior(self, initial_probes : Dict[Number, bool], lower : Optional[int],
            upper : Optional[int], invalid : bool) -> IntegerRangePrior:
        prior = IntegerRangePrior(self.fun, self.path, initial_probes)

        self.assertEqual(prior.search_strat.lower, lower,
            f"Unexpected lower bound chosen for initial probes {initial_probes}")
        self.assertEqual(prior.search_strat.upper, upper,
            f"Unexpected upper bound chosen for initial probes {initial_probes}")
        self.assertEqual(prior.is_invalid(), invalid,
    ...
    def test_init_range(self):
        ...
        self.init_prior({ -5 : True, -1 : True, 0 : False, 4 : True, 20 : False}, 4, 4, invalid=False)
        ...
        self.assertEqual(prior.search_strat.min_bound, -5,
            "Unexpected minimum lower bound chosen.")
        self.assertEqual(prior.search_strat.max_bound, self.max_int,
            "Unexpected maximum upper bound chosen.")

        prior = IntegerRangePrior(self.fun, self.path, { -5 : True, 4 : True, 20 : False})

        self.assertEqual(prior.search_strat.min_bound, self.min_int,
            "Unexpected minimum lower bound chosen.")
        self.assertEqual(prior.search_strat.max_bound, 20,
            "Unexpected maximum upper bound chosen.")

        prior = IntegerRangePrior(self.fun, self.path, { -5 : False, 4 : True, 20 : False, 40 : False})
    ...
    def test_isdone(self):
        ...
        self.assertFalse(prior.is_done(), "Prior should not be done if valid and not executed.")

        prior = self.init_prior({ -5 : False, 4 : False, 20 : False}, None, None, invalid=True)
        self.assertTrue(prior.is_done(), "Prior should be done if invalid after init.")

    def run_prior(self, target_lower : int, start_lower : int,
                  target_upper : int, start_upper : int,
                  initial_probes : Dict[Number, bool], invalid=False) -> IntegerRangePrior:
        prior = self.init_prior(initial_probes, start_lower, start_upper, invalid=invalid)
        while not prior.is_done():
            probe = prior.select_next_probe()
            result = ProbeResult(probe, None)

            self.assertTrue(isinstance(probe, StaticProbe), f"Probe expected to be StaticProbe but is {type(probe)}.")

            if (probe.get_probe_value() > target_upper or probe.get_probe_value() < target_lower):
                result.compile_ok = ExecutionResult.COMPILE_FAIL
                result.run_ok = ExecutionResult.NA
                result.verify_ok = ExecutionResult.NA
            else:
        ...
        self.assertEqual(prior.is_invalid(), invalid,
            "Expected prior to be valid.")

        if not invalid:
            self.assertEqual(prior.search_strat.state, BinaryBoundsSearch.BinSearchState.DONE,
                "Expected prior to be done.")
            self.assertEqual(prior.search_strat.upper, target_upper,
                f"Expected prior to find {target_upper} as upper bound.")
            self.assertEqual(prior.search_strat.lower, target_lower,
                f"Expected prior to find {target_lower} as lower bound.")
        else:
            self.assertEqual(prior.search_strat.upper, None,
                "Expected invalid prior to return None result for boundary.")
            self.assertEqual(prior.search_strat.lower, None,
                "Expected invalid prior to return None result for boundary.")
    ...
    def test_find_bounds(self):
        ...
        for lstart, ustart in start_bounds:
            self.run_prior(self.min_int,     lstart, self.max_int,    ustart,    {lstart : True, ustart : True})
            self.run_prior(self.min_int,     lstart, 3493496,    ustart,    {lstart : True, ustart : True})
            self.run_prior(-134217733,  lstart, self.max_int,    ustart,    {lstart : True, ustart : True})
            self.run_prior(-134217733,  lstart, 235332,     ustart,    {lstart : True, ustart : True})

            if lstart > 0:
                self.run_prior(0, lstart, self.max_int, ustart, {lstart : True, ustart : True})

        self.run_prior(0, 0, 0, 0, {0 : True})

    def run_verification_check(self, initial_probes : Dict[Number,bool],
        target_upper : Number, target_lower : Number,
        ...
        class IntegerBinaryBoundsSearch_DEBUG(IntegerBinaryBoundsSearch):
            ...
            def get_verification_targets(self, lower : Number, upper : Number) -> Optional[Iterable[Number]]:
                if self.counter in hacked_verify:
                    if VERBOSE: print(f"FOUND A HACKED VERIFICATION AT {self.counter}")
                ...
                if VERBOSE: print(f"Binary Search Verification started between {lower} and {upper} with probe targets {targets}")
                return targets


            def _set_state(self, state : BinaryBoundsSearch.BinSearchState):
                if VERBOSE: print(f"NEW STATE: {state}")
                super()._set_state(state)

        class IntegerRangePrior_DEBUG(IntegerRangePrior):
            def initialise_search(self, initial_values : Dict[Number,bool]) -> BinaryBoundsSearch:
        ...
        while not prior.is_done():
            probe = prior.select_next_probe()
            if VERBOSE: print(f"TESTING: {probe}")
            result = ProbeResult(probe, None)

            self.assertTrue(isinstance(probe, StaticProbe),
            ...
            if VERBOSE: print(f"RESULT: {result.is_exec_success()}")
    ...
    def test_find_bounds_verify_fail(self):
        self.run_verification_check({5 : True, 9 : True}, 499, -200,
             [(None,-201),(500,None)], {0:[23,55,105,110,150,200]})

        # adjust lower bound after verification
        self.run_verification_check({5 : True, 9 : True}, 499, 151,
             [(None,-201),(100,150),(500,None)], {0:[23,55,105,110,150,200]})

        # adjust upper bound after verification
        self.run_verification_check({5 : True, 9 : True}, 99, -200,
             [(None,-201),(100,150),(400,None)], {0:[23,55,105,110,150,200]})

        # return range where all numbers are tested in between
        self.run_verification_check({5 : True, 9 : True}, 14, 4,
             [(None,3),(15,18),(25,None)], {})
    ...
    def test_prior_result(self):
        ...
        self.assertEqual(result.result_data, f"{prior.search_strat.lower},{prior.search_strat.upper}")

        prior = self.run_prior(-134217733, None, 23599, None, {-35 : False, 34 : False}, invalid=True)
        result = prior.prior_result()
...
class TestRealRangeSearch(unittest.TestCase):
    ...
    def setUp(self):
        fn_data = FunctionData("add.cpp","_Z3addff",
                               "@$ f32, f32, f32 $@","add(float, float)",2,"instrument")
    ...
    def init_prior(self, initial_probes : Dict[Number, bool], lower : Optional[int],
            upper : Optional[int], invalid : bool) -> RealRangePrior:
    ...
    def test_init_range(self):
        ...
        self.init_prior({ -5.4 : False, -1.5 : True, 4.5 : True, 20.6 : False}, -1.5, 4.5, invalid=False)
        self.init_prior({ -5.4 : False, 4.5 : False, 20.6 : True}, 20.6, 20.6, invalid=False)

        # first diff (-5.4,0) = 5.4, second diff (0,20.6) = 20.6 --> 20.6
        self.init_prior({ -5.4 : True, -1.5 : True, 0 : False, 4.5 : True, 20.6 : False}, 4.5, 4.5, invalid=False)

        # test invalids
        self.init_prior({ -5.4 : False, 4.5 : False, 20.6 : False}, None, None, invalid=True)
        self.init_prior({ }, None, None, invalid=True)

        # test bound shortcuts
        prior = RealRangePrior(self.fun, self.path, { -5.4 : False, 4.5 : True, 20.6 : True})

        self.assertAlmostEqual(prior.search_strat.min_bound, -5.4,
            "Unexpected minimum lower bound chosen.")
        self.assertAlmostEqual(prior.search_strat.max_bound, self.max_pos,
            "Unexpected maximum upper bound chosen.")

        prior = RealRangePrior(self.fun, self.path, { -5.4 : True, 4.5 : True, 20.6 : False})

        self.assertAlmostEqual(prior.search_strat.min_bound, self.max_neg,
            "Unexpected minimum lower bound chosen.")
        self.assertAlmostEqual(prior.search_strat.max_bound, 20.6,
            "Unexpected maximum upper bound chosen.")

        prior = RealRangePrior(self.fun, self.path, { -5.4 : False, 4.5 : True, 20.6 : False, 40.7 : False})
    ...
    def test_isdone(self):
        prior = self.init_prior({ -5.4 : True, 4.5 : True, 20.6 : False}, -5.4, 4.5, invalid=False)
        self.assertFalse(prior.is_done(), "Prior should not be done if valid and not executed.")

        prior = self.init_prior({ -5.4 : False, 4.5 : False, 20.6 : False}, None, None, invalid=True)
        self.assertTrue(prior.is_done(), "Prior should be done if invalid after init.")
    ...
    def run_prior(self, target_lower : int, start_lower : int,
        ...
        if not invalid:
            self.assertEqual(prior.search_strat.state, BinaryBoundsSearch.BinSearchState.DONE,
                "Expected prior to be done.")
            self.assertTrue(
                math.isclose(prior.search_strat.upper, target_upper,
                    rel_tol=REAL_TOLERANCE * 10, abs_tol=REAL_TOLERANCE * 10),
                f"Expected prior to find {target_upper} as upper bound but found {prior.search_strat.upper}.")
            self.assertTrue(
                math.isclose(prior.search_strat.lower, target_lower,
                    rel_tol=REAL_TOLERANCE * 10, abs_tol=REAL_TOLERANCE * 10),
    ...
    def test_find_bounds(self):
        ...
        for lstart, ustart in start_bounds:
            self.run_prior(self.max_neg,     lstart, self.max_pos,    ustart,    {lstart : True, ustart : True})
            self.run_prior(self.max_neg,     lstart, 3493.496e23,    ustart,    {lstart : True, ustart : True})
            self.run_prior(-1342.17733e23,  lstart, self.max_pos,    ustart,    {lstart : True, ustart : True})
            self.run_prior(-134217.733e31,  lstart, 235.332e20,     ustart,    {lstart : True, ustart : True})

            if lstart > 0:
                self.run_prior(0, lstart, self.max_pos, ustart, {lstart : True, ustart : True})
    ...
    def test_find_bounds_verify_fail(self):
        ...
        # return range where all numbers are tested in between
        self.run_verification_check({5 : True, 9 : True}, 14.9, 3.01,
             [(None,3),(15,18),(25,None)], {})


        # run test for double data type
        fn_data = FunctionData("add.cpp","_Z3adddd",
                               "@$ f64, f64, f64 $@","add(double, double)",2,"instrument")
        ...
        self.run_verification_check({self.max_neg / 2 : True, self.max_pos / 2 : True}, 
    ...
    def test_prior_result(self):
        ...
        prior = self.run_prior(-324e20, None, 23599e12, None, {-35.3 : False, 34.4 : False}, invalid=True)
        result = prior.prior_result()
...
class TestIntegerOffsetSearch(unittest.TestCase):
    ...
    def init_prior(self) -> IntegerOffsetPrior:
        ...
        self.assertEqual(prior.state, BoundedPrior.SearchState.SEARCH_UPPER,
    ...
    def run_prior(self, target_lower : int, target_upper : int) -> IntegerOffsetPrior:
        ...
        while not prior.is_done():
            ...
            self.assertTrue(
                prior.state != BoundedPrior.SearchState.DONE and isinstance(probe, OffsetProbe),
                f"Probe not as expected but is {type(probe)}.")

            if (isinstance(probe, OffsetProbe) and
                ((prior.state == BoundedPrior.SearchState.SEARCH_UPPER and probe.get_probe_value() > target_upper) or
                 (prior.state == BoundedPrior.SearchState.SEARCH_LOWER and probe.get_probe_value() * -1 > target_lower))
                ):
                result.compile_ok = ExecutionResult.COMPILE_FAIL
                result.run_ok = ExecutionResult.NA
                result.verify_ok = ExecutionResult.NA
        ...
        self.assertEqual(prior.state, BoundedPrior.SearchState.DONE,
            "Expected prior to be done.")
        self.assertEqual(prior.upper_search_strat.final_bound, target_upper,
            f"Expected prior to find {target_upper} as upper bound.")
        self.assertEqual(prior.lower_search_strat.final_bound, target_lower,
            f"Expected prior to find {target_lower} as lower bound.")
    ...
    def run_verification_check(self,
        upper_max_bound : Number, upper_fail_vals : Iterable[Tuple[Number, Number]], upper_target : Number,
        lower_max_bound : Number, lower_fail_vals : Iterable[Tuple[Number, Number]], lower_target : Number,
        hacked_verify : Dict[Number,Iterable[Number]]):
        ...
        class IntegerOffsetPrior_DEBUG(IntegerOffsetPrior):
            ...
            def _get_verification_targets(self, search : BinarySearch) -> Optional[Iterable[Number]]:
                if self.counter in hacked_verify:
                    if VERBOSE: print(f"FOUND A HACKED VERIFICATION AT {self.counter}")
                    targets = hacked_verify[self.counter]
                else:
                ...
                if VERBOSE: print(f"Binary Search Verification started between 0 and {search.final_bound} with probe targets {targets}")
                return targets
        ...
        while not prior.is_done():
            ...
            if prior.state == BoundedPrior.SearchState.SEARCH_UPPER  or \
               prior.state == BoundedPrior.SearchState.UPPER_VERIFICATION:
                pvalue = probe.get_probe_value()
                valid_presult = pvalue <= upper_max_bound
                for start,end in upper_fail_vals:
                    valid_presult &= pvalue < start or pvalue > end
            elif prior.state == BoundedPrior.SearchState.SEARCH_LOWER or \
                 prior.state == BoundedPrior.SearchState.LOWER_VERIFICATION:
        ...
        self.assertEqual(prior.state, BoundedPrior.SearchState.DONE,
            "Expected prior to be done.")
        self.assertEqual(prior.upper_search_strat.final_bound, upper_target,
            f"Expected prior to find {upper_target} as upper bound.")
        self.assertEqual(prior.lower_search_strat.final_bound, lower_target,
            f"Expected prior to find {lower_target} as lower bound.")

    def test_find_bounds_verify_fail(self):
        self.run_verification_check(
            400, [(40,50)], 39, 300, [], 300,
            {0:[30,35,42,46,60,200]}
        )

        self.run_verification_check(
            400, [], 400, 5000, [(3100,3200)], 3099,
        ...
        fn_data = FunctionData("add.cpp","_Z3addii",
                               "@$ i64, i64, i64 $@","add(int, int)",2,"instrument")
        ...
        self.min_int = (-2**(self.path.type.bits-1))
        self.run_verification_check(
            3, [], 3, 9223372036854775808, [(7069083032380901136,7069083032380901136)], 7069083032380901135,
            {1:[8017592644271433344, 512785532786768516, 8223131078824675329,
            6284067516782266113, 6836160565201588871, 2848623699286436489,
            6718978620038693642, 7069083032380901136, 3998408631712509587,
            1694509785049988761, 3947920959936256411, 4908616807863520290,
            2079012785635087526, 7371593622109740469, 2798365133792951993,
            1153838086712650566, 339708350528082376, 3155119514743039560,
            6133607001479635788, 1910444492485076304, 187891479186685906,
            2239690827236022356, 8215133603322769875, 5030112268908400351,
            8242380280981873380, 6665714210786571496, 1249245793838910703,
            8800098160353328757, 5433908787818618363, 3072948510430934270]}
        )

        self.run_verification_check(
            3, [], 3, 9223372036854775808, [(8559802010813768848,8559802010813768848)], 8559802010813768847,
            {1:
            [335446990719811072, 6503277980993031424, 5212604295690172421, 
            3570768080391747980, 4223105336765619983, 7936273150500417807, 
            8559802010813768848, 8165961956204658833, 7754708996688390812, 
            3651416167080584110, 7609236531298307501, 1387822548415528495, 
            5585929138155763889, 5547659374701485239, 482348720901937727, 
            4677299792248478915, 7217083326301504708, 8173738490258911175, 
            1166326002164381010, 4300724910272297298, 4175446907632450644, 
            3852827712956724822, 7576973784238501847, 5025781789655560540, 
            6423333591157828958, 3807901971273448036, 3191678067490483180, 
            5504889308361584626, 4374163078889332981, 1381261653532159097]}
        )
...
class TestRealOffsetSearch(unittest.TestCase):
    ...
    def run_prior(self, target_lower : float, target_upper : float) -> RealOffsetPrior:
        ...
        while not prior.is_done():
            ...
            if ((prior.state == BoundedPrior.SearchState.SEARCH_UPPER and probe.get_probe_value() > target_upper) or
                 (prior.state == BoundedPrior.SearchState.SEARCH_LOWER and probe.get_probe_value() * -1 > target_lower)
                ):
                result.compile_ok = ExecutionResult.COMPILE_FAIL
                result.run_ok = ExecutionResult.NA
        ...
        self.assertTrue(
            math.isclose(prior.upper_search_strat.final_bound, target_upper,
                rel_tol=REAL_TOLERANCE * 10, abs_tol=REAL_TOLERANCE * 10),
                f"Expected prior to find {target_upper} as upper bound but found {prior.upper_search_strat.final_bound}.")
        self.assertTrue(
            math.isclose(prior.lower_search_strat.final_bound, target_lower,
                rel_tol=REAL_TOLERANCE * 10, abs_tol=REAL_TOLERANCE * 10),
    ...
    def test_find_bounds_verify_fail(self):
        ...
        self.run_verification_check(
            3.0, [(0.02,0.05)], 0.01, 300, [], 300,
            {0:[1.4,2.3,0.025,0.026]}
        )
...
class TestIntegerScaleSearch(unittest.TestCase):
    ...
    def run_prior(self, target_lower : int, target_upper : int) -> IntegerScalePrior:
        ...
        while not prior.is_done():
            ...
            self.assertTrue(
                prior.state != BoundedPrior.SearchState.DONE and isinstance(probe, ScaleProbe),
                f"Probe not as expected but is {type(probe)}.")

            if ((prior.state == BoundedPrior.SearchState.SEARCH_UPPER and probe.get_probe_value() - 1 > target_upper) or
                 (prior.state == BoundedPrior.SearchState.SEARCH_LOWER and 1 - probe.get_probe_value() > target_lower)
                ):
                result.compile_ok = ExecutionResult.COMPILE_FAIL
                result.run_ok = ExecutionResult.NA
                result.verify_ok = ExecutionResult.NA
    ...
    def run_verification_check(self,
        upper_max_bound : Number, upper_fail_vals : Iterable[Tuple[Number, Number]], upper_target : Number,
        lower_max_bound : Number, lower_fail_vals : Iterable[Tuple[Number, Number]], lower_target : Number,
        hacked_verify : Dict[Number,Iterable[Number]], custom_start = False, invalid = False):
