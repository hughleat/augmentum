                ...
                if (in.good()) {
                    ...
                    while (std::getline(in, line)) {
                        ...
                        auto process_token = [this](int tid, string token) {
                            if (tid == 0) target_modules.insert(token);
                            else if (tid == 1) target_functions.insert(token);
                        };
                    ...
                    errs() << "ERROR: [Augmentum] opening input stream to read target functions failed."
                        " Path invalid: " << target_spec << "\n";  
                }
                in.close();
