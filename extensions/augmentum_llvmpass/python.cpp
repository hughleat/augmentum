...
/**
 * Lots of LLVM types aren't copyable, and if the Python interpreter goes around deleting
 * them, bad things happen. So we wrap them in a smart pointer that doesn't delete.
 */
namespace pybind11 {
    namespace detail {
        template<typename T>
...
namespace augmentum {
    namespace llvmpass {
            ...
            py::class_<Function, NoDel<Function>>(m, "Function")
                ...
                .def("__repr__", 
                    [](const Function& function) { 
                        ...
                        return "<llvm.Function named '" + function.getName().str() + "' in module '" + mod_name + "'>"; 
                    }
                )
                .def("get_signature",
                    [](const Function& function) {
        ...
        // Callbacks from C++
        // VS: The visibility attribute fixes a visibility warning where ShouldInstrument had
        // lower visibility in the final library than PythonShouldInstrument
        // see here: https://stackoverflow.com/questions/2828738/c-warning-declared-with-greater-visibility-than-the-type-of-its-field
