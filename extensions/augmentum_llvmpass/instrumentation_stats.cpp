...
namespace augmentum {
    namespace llvmpass {
        ...
        static const string fun_stats_out_head = "MODULE" + stats_out_delim + 
                                                "FNAME" + stats_out_delim +
                                                "FNAMED" + stats_out_delim +
                                                "ICOUNT" + stats_out_delim +
                                                "PCOUNT" + stats_out_delim +
                                                "FUNCTIONTY" + stats_out_delim +
                                                "CAN_INSTR" + stats_out_delim +
                                                "SHOULD_INSTR";
        static const string named_struct_stats_out_head = "MODULE" + stats_out_delim + 
                                                "STRUCT_NAME" + stats_out_delim +
                                                "TYPE" + stats_out_delim +
                                                "LLVM_NAME" + stats_out_delim +
                                                "EXTRA";

        void InstrumentationStats::record_function_stats(const Module& module, const Function& function, 
            ...
            if (function_statistics.find(fname) == function_statistics.end()) {
                ...
                if (full_stats) {
                    ...
                    FunctionData data {
                        ...
                        type_serialiser.serialise_type(module, function, function.getFunctionType(), SerialisationContext::FUNCTION),
                        instr_info.first,
                        instr_info.second
                    };
        ...
        void InstrumentationStats::record_named_struct_stats(const Module& module) {
            ...
            for (auto&[name, type] : type_serialiser.named_structs()) {
                if (named_struct_statistics.find(name) == named_struct_statistics.end()) {
                    ...
                    named_struct_statistics[name] = {
                        module.getName().str(),
                        stype->getName().str(),
                        serialised_type,
                        type_to_string(stype),
                        extra
                    };
                }
            }
        ...
        void InstrumentationStats::emit_statistics(const string& outDir, const string& prefix) const {
            ...
            if (filesystem::exists(outDirP)) {
                ...
                emit_stats(outDirP / (prefix + "_" + fun_stats_out_file_name), fun_stats_out_head,
                    [this](std::ofstream& out){
                        for (auto&[fname, entry] : function_statistics) {
                            out << escape_and_delim(entry.module_name)
                                << escape_and_delim(entry.function_name)
                                << escape_and_delim(entry.function_name_demangled)
                                << escape_and_delim(entry.instruction_count >= 0 ? to_string(entry.instruction_count) : "NA")
                                << escape_and_delim(entry.parameter_count >= 0 ? to_string(entry.parameter_count) : "NA")
                                << escape_and_delim(entry.type_serialisation)
                                << escape_and_delim(entry.can_instr)
                ...
                emit_stats(outDirP / (prefix + "_" + named_struct_out_file_name), named_struct_stats_out_head,
                    [this](std::ofstream& out){
                        for (auto&[name, entry] : named_struct_statistics) {
                            out << escape_and_delim(entry.module_name)
                                << escape_and_delim(entry.struct_name)
                                << escape_and_delim(entry.serialised_type)
                                << escape_and_delim(entry.llvm_name)
                                << escape_and_delim(entry.extra, false)
                                << "\n";
                        }
                    }
                );
