...
namespace {
    /**
    * Making my own list. Dealing with the circular references in std::list refering to iteself, was driving me made.
    * This is actually quicker to write than working out the stl way.
    * The erase function would be easier and faster if this was a doubly linked list, but I don't think they should
    * get called very often, nor should the list ever be very deep, so I think it's better to no pay the space cost.
    */
    typedef void* Handle;
    ...
    struct ListNode {
        ...
        AdviceId id;
        ListNode* next;
        ListNode(Function function, AdviceId id, ListNode* next): function(function), id(id), next(next) {}
        operator Function&() {
            return function;
        }
    };
    ...
    struct ListIt {
        ...
        bool operator ==(ListIt<Function> that) {
    ...
    struct List {
        ...
        void push_front(Function function, AdviceId id) {
            head = new ListNode(function, id, head);
        }
        bool empty() {
            return head == nullptr;
        }
        ListIt<Function> begin() {
            return { head };
        }
        ListIt<Function> end() {
            return { nullptr };
        ...
        void remove(AdviceId id) {
            assert(id != 0); // Should already have been checked in the FnExtensionPoint::remove functions.
            ListNode<Function>* prev = nullptr;
            ListNode<Function>* curr = head;
    ...
    */
    struct ExtensionData {
        ...
        bool empty() {
            return befores.empty() && arounds.empty() && afters.empty();
        }
    };
