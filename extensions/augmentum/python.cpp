...
namespace augmentum {
    namespace python {
        ...
        struct Allocation {
            ...
            ~Allocation() {
                delete [] reinterpret_cast<int8_t*>(address);
            }
            intptr_t address;
        };
        ...
        // Initialise the "augmentum.impl" Python module which links the Python and C++ code.
        PYBIND11_EMBEDDED_MODULE(augmentum, m) {
            auto impl = m.def_submodule("impl");
            impl.def("get_unique_advice_id", get_unique_advice_id) // Note this one is already in the augmentum namespace
...
#               include "augmentum.py"
            ...
            /**
             * Flag to indicate if the user decided to extend with Python so we can clear up later.
             */
            bool added = false;
            ...
            PythonMain(const char* module_name) {
                if (module_name) {
                    ...
                    py::exec(augmentum_script, py::reinterpret_borrow<py::dict>(py_augmentum->attr("__dict__").ptr()));
                ...
                if (it == py_type_descs.end()) {
                    ...
                    switch (type_desc->get_discriminator()) {
                        case TypeDesc::UNKNOWN: {
                            ...
                            py_type_descs[key] = py_TypeDesc(sig);
                        }
                        break;
                        case TypeDesc::FUNCTION: {
                            ...
                            py_type_descs[key] = py_FnTypeDesc(sig, get_py_type(fn_type_desc->get_return_type()), py_arg_types);
                        }
                        break;
                        default:
        ...
        namespace impl {
            ...
            void extend_around(py::object py_pt, py::object py_advice, AdviceId id) {
                auto& pt = *get_pt(py_pt);
                auto advice = [py_advice, py_pt](FnExtensionPoint& pt, AroundHandle handle, RetVal ret_val, ArgVals arg_vals) {
