...
namespace augmentum {
    ...
    struct UnknownTypeDesc: TypeDesc {
        ...
        Discriminator get_discriminator() const {
            return UNKNOWN;
        }
        static UnknownTypeDesc* get(std::string module, std::string signature);
    private:
        UnknownTypeDesc(std::string module, std::string signature): module(module), signature(signature) {}
        std::string module;
    ...
    struct IntTypeDesc: TypeDesc {
        ...
        static IntTypeDesc* get_i1() { 
            return &i1_type; 
        }
        static IntTypeDesc* get_i8() {
            return &i8_type;
        }
        static IntTypeDesc* get_i16() {
            return &i16_type;
        }
        static IntTypeDesc* get_i32() {
            return &i32_type;
        }
        static IntTypeDesc* get_i64() {
            return &i64_type;
        }
    private:
    ...
    struct FloatTypeDesc: TypeDesc {
        ...
        static FloatTypeDesc* get_float() {
            return &float_type;
        }
        static FloatTypeDesc* get_double() {
            return &double_type;
        }
    private:
    ...
    struct PointerTypeDesc: TypeDesc {
        std::string get_signature() const {
            return element_type->get_signature() + "*";
        }
        Discriminator get_discriminator() const {
            return POINTER;
        }
        TypeDesc* get_element_type() const {
            return element_type;
        }
    ...
    struct SequentialTypeDesc: TypeDesc {
        TypeDesc* get_contained_type() const {
    ...
    protected:
        SequentialTypeDesc(TypeDesc* contained_type, size_t num_elems): contained_type(contained_type), num_elems(num_elems) {}
    private:
        size_t num_elems;
    ...
    struct ArrayTypeDesc: SequentialTypeDesc {
        std::string get_signature() const {
            return "[" + std::to_string(get_num_elems()) + 
                   " x " + get_contained_type()->get_signature() + "]";
        }
        Discriminator get_discriminator() const {
            return ARRAY;
        }
    ...
    private:
        ArrayTypeDesc(TypeDesc* contained_type, size_t num_elems): SequentialTypeDesc::SequentialTypeDesc(contained_type, num_elems) {}
    };
    ...
    private:
        VectorTypeDesc(TypeDesc* contained_type, size_t num_elems): SequentialTypeDesc::SequentialTypeDesc(contained_type, num_elems) {}
    };
    ...
    struct StructTypeDesc: TypeDesc {
        ...
        TypeDesc* get_elem_type(size_t i) const {
            return elems[i];
        }
        const std::vector<TypeDesc*> get_elem_types() const {
            return elems;
        }
        /**
         * Determine if this is currently a forward declaration - i.e. that the element types have not been set.
         */
         ...
         * If the struct already exists and is not forward, then the element types must match.
         * If the struct already exists and is forward, it will be defined and no longer be forward.
         */ 
        static StructTypeDesc* get_named(std::string module, std::string name, std::vector<TypeDesc*> elem_types);
        /** 
         * Get a forward declaration struct.
         */
        static StructTypeDesc* get_forward(std::string module, std::string name);
    private:
        StructTypeDesc(std::string module, std::string name, std::vector<TypeDesc*> elems, bool forward): module(module), name(name), elems(elems), forward(forward) {}
        std::string module;
        std::string name;
    ...
    struct FnTypeDesc: TypeDesc {
        ...
        TypeDesc* get_arg_type(size_t i) const {
            return args[i];
        }
    ...
    private:
        FnTypeDesc(TypeDesc* return_type, std::vector<TypeDesc*> args): return_type(return_type), args(args) {}
    ...
    inline PointerTypeDesc* TypeDesc::get_ptr() {
        return PointerTypeDesc::get(this);
    }
