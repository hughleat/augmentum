...
struct AddOneListener: Listener {
    AroundAdvice add_one = [this](FnExtensionPoint& pt, AroundHandle handle, RetVal ret_value, ArgVals arg_values) {
...
struct AddOnePointerListener: Listener {
    AroundAdvice add_one = [this](FnExtensionPoint& pt, AroundHandle handle, RetVal ret_value, ArgVals arg_values) {
        ...
        const TypeDesc* type = reinterpret_cast<const PointerTypeDesc*>(pt.get_return_type())->get_element_type();
    ...
    void on_extension_point_register(FnExtensionPoint& pt) {
        if (pt.get_return_type()->get_discriminator() == TypeDesc::POINTER) {
            const TypeDesc* elem_type = reinterpret_cast<const PointerTypeDesc*>(pt.get_return_type())->get_element_type();
...
struct AddOneFirstParameterListener: Listener {
    ...
    void on_extension_point_register(FnExtensionPoint& pt) {
        ...
        if (pt.get_num_args() >= 1 && pt.get_arg_type(0)->get_discriminator() == TypeDesc::POINTER) {
            const TypeDesc* elem_type = reinterpret_cast<const PointerTypeDesc*>(pt.get_arg_type(0))->get_element_type();
            // first arg is pointer to integer type
            if (elem_type->get_discriminator() == TypeDesc::INT) {
                REG_LOG("AddOneFirstParameterListener extending " << pt.get_name() << " " << pt.get_signature());
                pt.extend_around(add_one, id);
            }
        }
    }
    void on_extension_point_unregister(FnExtensionPoint& pt) {
        REG_LOG("AddOneFirstParameterListener unextending " << pt.get_name() << " " << pt.get_signature());
...
struct AddOneStructListener: Listener {
    ...
    void on_extension_point_register(FnExtensionPoint& pt) {
        ...
        if (pt.get_return_type()->get_discriminator() == TypeDesc::STRUCT) {
            const StructTypeDesc* struct_type = reinterpret_cast<const StructTypeDesc*>(pt.get_return_type());
...
struct AddOneNamedStructListener: Listener {
    ...
    void on_extension_point_register(FnExtensionPoint& pt) {
        ...
        if (return_type->get_discriminator() == TypeDesc::POINTER &&
            reinterpret_cast<const PointerTypeDesc*>(return_type)->get_element_type()->get_discriminator() == TypeDesc::STRUCT) {
            ...
            if (struct_type->get_num_elems() >= 1 &&
                struct_type->get_elem_type(0)->get_discriminator() == TypeDesc::INT) {
                REG_LOG("AddOneNamedStructListener extending " << pt.get_name() << " " << pt.get_signature());
